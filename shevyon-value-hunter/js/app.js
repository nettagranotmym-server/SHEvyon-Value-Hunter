...
function renderDecisionScreen() {
  ...
  document.getElementById("gStep").textContent = isTraining ? "שנת ניסיון" : `שנה ${data.year} מתוך 5`;
  ...
  const cards = COMPANIES.map(c => {
    ...
    return `
      <article class="company-card">
        ...
        <div class="metrics">
          <div><span>הכנסות</span><b>${d.revenue}</b></div>
          <div><span>רווח</span><b>${d.profit}</b></div>
        </div>
        ...
        <div class="alloc-row">
          <button class="pct-btn" data-action="minus" data-key="${c.key}">−10%</button>
          <div class="pct-val" id="pct_${c.key}">${S.alloc[c.key]}%</div>
          <button class="pct-btn" data-action="plus" data-key="${c.key}">+10%</button>
        </div>
      </article>
    `;
  }).join("");
  ...
  document.getElementById("gContent").innerHTML = `
    ${isTraining ? `<div class="training-note">זו שנת תרגול בלבד — התוצאה לא תשפיע על המשחק.</div>` : ""}
    <div class="companies-grid">${cards}</div>
    <div class="alloc-warning" id="allocWarn"></div>
    <button class="cbtn" id="confirmBtn">${isTraining ? "סיימנו תרגול ➡️" : "אישור החלטה ➡️"}</button>
  `;

  document.querySelectorAll(".pct-btn").forEach(btn => {
    btn.addEventListener("click", () => changeAlloc(btn.dataset.key, btn.dataset.action));
  });
...
function changeAlloc(key, action) {
  const delta = action === "plus" ? 10 : -10;
  if (delta > 0) {
    const others = COMPANIES.filter(c => c.key !== key && S.alloc[c.key] >= 10);
    if (!others.length || S.alloc[key] > 90) return;
    S.alloc[key] += 10;
    others[0].key === key ? null : S.alloc[others[0].key] -= 10;
  } else {
    if (S.alloc[key] < 10) return;
    const target = COMPANIES.find(c => c.key !== key && S.alloc[c.key] <= 90);
    if (!target) return;
    S.alloc[key] -= 10;
    S.alloc[target.key] += 10;
  }
  refreshAllocUI();
}
...
function refreshAllocUI() {
  COMPANIES.forEach(c => {
    const el = document.getElementById(`pct_${c.key}`);
    if (el) el.textContent = S.alloc[c.key] + "%";
  });
  const sum = COMPANIES.reduce((s, c) => s + S.alloc[c.key], 0);
  document.getElementById("allocWarn").textContent = sum === 100 ? "" : `החלוקה צריכה להיות 100%, כרגע ${sum}%`;
  document.getElementById("confirmBtn").disabled = sum !== 100;
}
...
function applyCurrentYear() {
  ...
  const previousStep = S.history.length === 0 ? TRAINING_YEAR : YEARS[S.history.length - 1];
  ...
  COMPANIES.forEach(c => {
    ...
    newTotal += after;
    details.push({ ...c, ret, before, after });
  });
  ...
  upsertTeamProgress({
    ...
    totalNow: S.total,
    yearTotals: S.history.map(h => ({ year: h.year, totalAfter: h.totalAfter }))
  });
...
function fixAllocRounding() {
  const sum = COMPANIES.reduce((s, c) => s + S.alloc[c.key], 0);
  if (sum !== 100) S.alloc[COMPANIES[0].key] += (100 - sum);
}
...
function showYearResult(yearData, details, oldTotal) {
  ...
  document.getElementById("gContent").innerHTML = `
    <div class="yrc">
      ...
      <div class="yr-i">${positive ? "📈" : "📉"}</div>
      <div class="yr-ch ${positive ? "pos" : "neg"}">${positive ? "+" : ""}${change.toFixed(1)}%</div>
      <div class="yr-ex">${yearData.lesson}</div>
      <div class="yr-bd">
        ${details.map(d => `
          <div class="yr-ar">
            <span>${d.icon} ${d.name}</span>
            <span class="${d.ret >= 0 ? "pos" : "neg"}">${d.ret >= 0 ? "+" : ""}${(d.ret * 100).toFixed(0)}%</span>
          </div>
        `).join("")}
      </div>
      <div class="yr-bal">
      ...
      <button class="nbtn" id="nextBtn">${yearData.year >= 5 ? "לתוצאות הסופיות 🏆" : "לשנה הבאה ➡️"}</button>
    </div>
  `;
...
function showWaiting(step) {
  ...
  document.getElementById("gContent").innerHTML = `
    <div class="evc wait-card">
      <div class="evi">⏳</div>
      <div class="evt">ממתינות לפתיחת ${label}</div>
      <div class="evd">המנהלת תפתח את השלב הבא בקרוב.<br>אין צורך לרענן את הדף.</div>
      <div id="waitDots" class="admin-muted">בודקות...</div>
    </div>
  `;
...
function showResults() {
  ...
  document.getElementById("rReturn").textContent = `${positive ? "+" : ""}${totalReturn.toFixed(0)}% תשואה כוללת`;
  document.getElementById("rReturn").className = `fr ${positive ? "pos" : "neg"}`;

  const avg = {};
  COMPANIES.forEach(c => {
    avg[c.key] = Math.round(S.avgAllocSamples.reduce((s, a) => s + (a[c.key] || 0), 0) / Math.max(1, S.avgAllocSamples.length));
  });
  ...
  const perf = COMPANIES.map(c => {
    ...
    return { ...c, perf: (end / start - 1) * 100 };
  }).sort((a, b) => b.perf - a.perf);

  document.getElementById("winnerCard").innerHTML = `
    <div class="bonus-title">החברה שיצרה הכי הרבה ערך</div>
    <div class="bonus-val">${perf[0].icon} ${perf[0].name}</div>
    <div class="bonus-text">למרות הכותרות המפחידות בתחילת הדרך, העסק התחזק לאורך זמן.</div>
  `;

  document.getElementById("rDetails").innerHTML = `
    <div class="det-t">החזקה ממוצעת לאורך המשחק:</div>
    ${COMPANIES.map(c => `<div class="det-r"><span>${c.icon} ${c.name}</span><b>${avg[c.key]}%</b></div>`).join("")}
    <div class="det-t">מה קרה בפועל לחברות:</div>
    ${perf.map(c => `<div class="det-r"><span>${c.icon} ${c.name}</span><b class="${c.perf >= 0 ? "pos" : "neg"}">${c.perf >= 0 ? "+" : ""}${c.perf.toFixed(0)}%</b></div>`).join("")}
  `;
